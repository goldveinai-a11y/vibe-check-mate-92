...
export function ReportChat({ analysisId, ownerAnonId }: Props) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (res) => {
      if ("error" in res) {
        if (res.error === "limit_reached") setError("You've used all your questions for this report.");
    ...
    <section className="mt-12 rounded-3xl border border-border/60 bg-card p-6 shadow-sm sm:p-8">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-serif text-2xl leading-tight">Ask the report</h3>
            <p className="mt-1 text-sm text-ink/60">
              Follow-up questions, grounded in your data.
            </p>
          </div>
        </div>
        <div className="shrink-0 rounded-full bg-cream px-3 py-1 text-[11px] font-medium text-ink/70 tabular-nums">
      ...
      >
        <textarea
          ...
          maxLength={400}
          rows={1}
          placeholder={remaining <= 0 ? "You've used all your questions." : "Ask a follow-up…"}
...
function Bubble({ role, content }: { role: "user" | "assistant"; content: string }) {
  ...
  return (
    <div className="max-w-[85%] self-start whitespace-pre-wrap rounded-2xl bg-purple-soft/60 px-4 py-2 text-sm text-ink">
      {content}
    </div>
  );
}
